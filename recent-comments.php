	...
	function execute($args='', $default_output_template='<li>{commentlink}</li>'){
		...
		if (0 < $options['limit']) {	
			...
			if (!$unique) {
				...
				$sql = "SELECT * FROM ( SELECT comment_post_ID, MAX(comment_date) as max_date FROM $wpdb->comments GROUP BY comment_post_ID ) AS x ";
				$sql .= "INNER JOIN $wpdb->comments AS f ON f.comment_post_ID = x.comment_post_ID AND f.comment_date = x.max_date ";
				$sql .= "LEFT JOIN $wpdb->posts ON ID = x.comment_post_ID ";
			}
		    if ($check_custom) {
